...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Cooperation trajectories across N=3 replications. Condition C (red) starts below A and B but converges to 100\% cooperation by tick 120 in all three runs. After convergence, C remains at or near 100\% for the rest of the run.}]{figures/cooperation_trajectory.png}}
...
\textbf{Code:} \texttt{rust\_atom/src/donor\_agent.rs} and
\texttt{rust\_atom/src/donor\_bench.rs}. The architecture used in Runs 6
through 9 is at commit \texttt{dcb5f38}; the repository is currently
private and will be released alongside the preprint.
...
\emph{Independent research, no funding. Code will be released with the
